...
abstract: |
  Text of abstract
keywords: |
  Funnel beaker; Entropy Maximization; Landscape Archaeology
highlights: |
...
$$
D_j = \sum_i \frac{O_i * W_j * c_{ij}^-\beta}{\sum_k W_k * c_{ik}^-\beta}
$$

[@rihll_modelling_1988, Mathematical Appendix]

```{r moin1}
library(moin)

int_mod_del <- sc(Oi = rep(x = 10, times = length(sett)),
                  Wj = rep(x = 5, times = length(sett)),
                  cij = distmat_del,
                  beta = 1,
                  detfun = "power")
sett@data$Dj_del <- int_mod_del$Dj

int_mod_gab <- sc(Oi = rep(x = 10, times = length(sett2)),
                  Wj = rep(x = 5, times = length(sett2)),
                  cij = distmat_gab,
                  beta = 1,
                  detfun = "power")
sett2@data$Dj_gab <- int_mod_gab$Dj

library(ggplot2)
plot_del <- sett %>%
  sf::st_as_sf() %>%
  ggplot(aes(size = Dj_del, color = Dj_del)) + 
  geom_sf() +
  theme_bw() +
...
plot_gab <- sett2 %>%
  sf::st_as_sf() %>%
  ggplot(aes(size = Dj_gab, color = Dj_gab)) + 
  geom_sf() +
  theme_bw() +
  guides(color=guide_legend(), size = guide_legend())
...
int_mod_del_deg <- sc(Oi = rep(x = 10, times = length(sett)),
                      Wj = sett@data$del_degree,
                      cij = distmat_del,
                      beta = 1,
                      detfun = "power"
                      )
sett@data$Dj_del_deg <- int_mod_del_deg$Dj

mapview::mapview(sett["Dj_del_deg"],
                 cex = sett$Dj_del_deg,
                 zcol = "Dj_del_deg")
...
int_mod_gab_deg <- sc(Oi = rep(x = 10, times = length(sett2)),
                      Wj = sett2@data$gab_degree,
                      cij = distmat_gab,
                      beta = 1,
                      detfun = "power"
                      )
sett2@data$Dj_gab_deg <- int_mod_gab_deg$Dj

mapview::mapview(sett2["Dj_gab_deg"],
                 cex = sett2$Dj_gab_deg,
                 zcol = "Dj_gab_deg")
```
...
for (i in 1:length(beta_seq)) {
  ...
  beta_test <- rbind(beta_test, data.frame(beta = rep(j, length(sett)),
                                           Dj = sc(Oi = rep(x = 10, times = length(sett)),
                                                   Wj = sett@data$del_degree,
                                                   cij = distmat_del,
                                                   beta = j,
                                                   detfun = "power"
...
ggplot(beta_test2, aes(x = Dj, y = as.factor(beta), fill = as.factor(degree))) + 
  ggjoy::geom_joy(scale = 2, rel_min_height=.01, alpha = .8, color = "white") +
  ggjoy::theme_joy(font_size = 8, line_size = .5) +
